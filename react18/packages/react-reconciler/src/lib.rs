...
impl Reconciler {
    ...
    pub fn create_container(&self, _container: &JsValue) -> Rc<RefCell<FiberRootNode>> {
        Rc::new(RefCell::new(FiberRootNode {}))
    }

    pub fn update_container(&self, element: Rc<JsValue>, _root: Rc<RefCell<FiberRootNode>>) {
        ...
        let text_instance = self
            .host_config
            .create_text_instance(children.as_string().unwrap());
        let div_instance = self.host_config.create_instance(_type.as_string().unwrap());
        self.host_config
            .append_initial_child(div_instance.clone(), text_instance);
        let window = window().unwrap();
        let document = window.document().unwrap();
        let body = document.body().expect("document should have a body");
        let _ = body.append_child(&*div_instance.clone().downcast::<Element>().unwrap());
    }
}
